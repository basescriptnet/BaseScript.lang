            ...
            switch (statement.type) {
                ...
                case 'string':
                    if (/\r\n/.test(value) || /\$\{/.test(value)) {
                        return `\`${value.replace(/\'/g, '\x27').replace(/"/g, '\x22')/*.replace(/(?:(?:[^\$\\]|(?:\\\\|\\\$)+)(\$))([A-Za-z$_][A-Za-z$_0-9]*)/gmi, `" + $2 + "`)*/.replace(/\`/g, '\x60')}\``
                    }
                    //result += JSON.stringify(value)
                    return `"${value.replace(/\'/g, '\x27').replace(/"/g, '\x22')/*.replace(/(?:(?:[^\$\\]|(?:\\\\|\\\$)+)(\$))([A-Za-z$_][A-Za-z$_0-9]*)/gmi, `" + $2 + "`)*/.replace(/\`/g, '\x60')}"`//value.indexOf('$') > -1 ? `\`${value}\`` : `${JSON.stringify(value)}`;
                case 'null':
                    return null;
                case 'new.target':
                    return 'new.target';
