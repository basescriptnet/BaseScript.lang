            ...
            switch (statement.type) {
                ...
                case 'string':
                    if (/\r\n/.test(value) || /\$\{/.test(value)) {
                        return `${statement.quoteType}${value.replace(/\'/g, '\x27').replace(/"/g, '\x22')/*.replace(/(?:(?:[^\$\\]|(?:\\\\|\\\$)+)(\$))([A-Za-z$_][A-Za-z$_0-9]*)/gmi, `" + $2 + "`)*/.replace(/\`/g, '\x60')}${statement.quoteType}`
                    }
                    //result += JSON.stringify(value)
                    return `${statement.quoteType}${value.replace(/\'/g, '\x27').replace(/"/g, '\x22')/*.replace(/(?:(?:[^\$\\]|(?:\\\\|\\\$)+)(\$))([A-Za-z$_][A-Za-z$_0-9]*)/gmi, `" + $2 + "`)*/.replace(/\`/g, '\x60')}${statement.quoteType}`//value.indexOf('$') > -1 ? `\`${value}\`` : `${JSON.stringify(value)}`;
                case 'null':
                    return null;
                case 'new.target':
                    return 'new.target';
                case 'safeValue':
                    ...
                    return `(function () { try { ${v} } catch (e) { return void(0); } return ${v}; })()`;
                case 'defined':
                ...
                case 'object':
                    ...
                    for (let i in value) {
                        if (value[i].type == 'es6_key_value') {
                            ...
                            result += `"${i}":${parsePrimitive(value[i])},`;
                        } else {
