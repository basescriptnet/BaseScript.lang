...
const libs = {
    ...
    get Math() {
        return 'builtin/Math.js'
    },
    get random() {
        return 'builtin/random.js'
    },
    get Date() {
        return 'builtin/Date.js'
    }
}
let lvl = 0;
module.exports = function parse (statements, tmp, isReturn, caller, prependStr = '') {
    ...
    for (let i = 0; i < statements.length; i++) {
        ...
        switch (statement.type) {
            ...
            case 'var_reassign':
                if (tmp == 'const') {
                    result += `$${lvl}.set("${statement.identifier.value}", BS.deepFreeze(${parse(value)}), true, true)`;
                    break;
                }
                //if (tmp == 'Int') {
                ...
                if (typeof statement.identifier.value != 'string')
                    ...
                    result += `$${lvl}.set("${statement.identifier.value}", ${parse(value)}${['Int', 'Array', 'Object', 'let', '\\'].includes(tmp) ? ', true' : ''})`;
                break;
            case 'array':
                result += 'BS.Array([';
                for (let i = 0; i < value.length; i++) {
            ...
            case 'debugging': {
                ...
                if (statement.method == 'log' || statement.method == 'error') {
                    result += `console.${statement.method}(get_clear_value(${v}));`
                } else {
                    //let v = parse(value);
            ...
            case 'dot_retraction_v2':
                ...
                if (tmp == 'delete') {
                    if (statement.from.type != 'identifier') {
                        result += `$${lvl}.set(${from}, $delete(${from}, "${value.value}"));`;
                        break;
                    }
                    result += `$${lvl}.set("${statement.from.value}", $delete(${from}, "${value.value}"));`;
                    break;
                }
            ...
            case 'annonymous_function': {
                ...
                res += `(${t.length ? t[0] : ''}) {
                    ${parse(statement.value, statement.text, null, 'function',
                        ...
                        +`$${lvl+1}.set("this", $${lvl+1}.variables, true, true, true);`
                        + `${t.length ? t[1] + '\n' : ''}`
                        +`${t[2] ? t[2] : ''}`)
                    }
                    return null;
                ...
                if (caller == 'statement' && statement.identifier) {
                    res = `$${lvl}.set("${statement.identifier.value}", ${res})`
                }
                if (statement.identifier) {
                    result += res;
                    break;
            ...
            case 'return':
                if (value === void 0) {
                    value = `$return($${lvl}, scopes, null)`;
                } else {
                    value = `$return($${lvl}, scopes, ${parse(value, tmp, isReturn)})`;
                }
                var t = '';
                // switch (tmp) {
                //     case 'int':
            ...
            case 'identifier':
                ...
                if (tmp == 'realValue') {
                    result += `$${lvl}.get("${value}", 1)`;
                    break;
                }
                result += `$${lvl}.get("${value}")`;
                //result += value;
                break;
            case 'global':
            ...
            case 'function_call':
                if (value.type == 'identifier' || value.type == 'keyword') {
                    ...
                    result += `$get(${parse(value.from, tmp)}, `
                }
            ...
            case 'for_loop':
                ...
                if (!/^\d+$/.test(value0) || !/^\d+$/.test(value1)) {
                    result += `for (let _i${iterator} of range(${value0}, ${value1}, ${statement.include})) {
                        ${parse(value, tmp, null, '', `$${lvl+1}.set("${statement.identifier.value}", _i${iterator}, true);`)}
                    }`
                    break;
                }
            ...
            case 'expression_short_equation': {
                let v = parse(value[0], 'realValue');
                if (value[0].type == 'identifier') {
                    if (value[1].value == '+') {
                        result += `$${lvl}.set("${value[0].value}", $add(${v}, ${parse(value[2])}))`;
                        break;
                    }
                    result += `$${lvl}.set("${value[0].value}", ${v} ${value[1].value} ${parse(value[2])})`;
                    break;
                }
                let parentesis = false;
                let type = value[0].type;
                if (type == 'item_retraction' || type == 'dot_retraction_v2') {
                    parentesis = true;
                    let prop = '';
                    if (value[0].value && (value[0].value.type == 'identifier' || value[0].value.type == 'keyword')) {
                        prop = `"${value[0].value.value}"`;
                    } else {
                        prop = parse(value[0].value, 'realValue');
                    }
                    result += `$update(${parse(value[0].from, 'realValue')}, ${prop}, `
                    if (value[1].value == '+') {
                        result += `$add(${v}, ${parse(value[2])}))`;
                        break;
                    }
                    result += `${v} ${value[1].value} ${parse(value[2])})`
                    break
                } else if (type == 'item_retraction_last') {
                    let val = parse(value[0].from, 'realValue');
                    result += `$update(${val}, BS.last(${val}.length), `;
                    if (value[1].value == '+') {
                        result += `$add(${v}, ${parse(value[2])}))`;
                        break;
                    }
            ...
            case 'sum':
                ...
                if (statement.operator == '+') {
                    result += `$add(${parse(statement.left)}, ${parse(statement.right)})`;
                    break;
                }
            ...
            case 'item_retraction':
                if (tmp == 'delete') {
                    ...
                    result += `$${lvl}.set(${from}, $delete(${parse(statement.from)}, "${v}"));`;
                } else {
            ...
            case 'arguments_with_types':
                ...
                for (let i = 0; i < sav.length; i++) {
                    ...
                    args.push(`$${lvl+1}.set("${sav[i].value}", ${sav[i].value});`)
                }
