...
function parseVar(statement, tmp, isReturn, caller, prepend = '') {
    ...
    switch (statement.type) {
        case 'var_assign':
            ...
            if (tmp == 'isGloobal')
                pr = 'var';
            return `${pr} ${parse(value, prepend || '', prepend && prepend.trim ? prepend.trim() : prepend)};`;
...
function parse (statements, tmp, isReturn, caller, prepend = '') {
    ...
    for (let i = 0; i < statements.length; i++) {
        ...
        var r = [];
        // primitives
        if (['array', 'array_through', 'number', 'bigInt', 'string', 'null', 'boolean', 'regexp', 'new.target', 'object'].indexOf(statement.type) > -1) {
