...
let namespaces = [];
module.exports = function parse (statements, tmp, isReturn, caller, prepend = '') {
        ...
        switch (statement.type) {
            ...
            case 'scope': {
                ...
                if (namespace_count < namespaces.length) {
                    namespaces.splice(namespace_count, 1);
                }
            ...
            case 'var_assign':
                ...
                if (tmp == 'isGloobal')
                    pr = 'var';
                result += `${pr} ${parse(value, prepend || '', prepend && prepend.trim ? prepend.trim() : prepend)};`;
                break;
            ...
            case 'string':
                if (/\r\n/.test(value)) {
                    result += `\`${value.replace(/\'/g, '\x27').replace(/"/g, '\x22').replace(/\`/g, '\x60')}\``
                    break;
                }
                //result += JSON.stringify(value)
                result += `"${value.replace(/\'/g, '\x27').replace(/"/g, '\x22').replace(/\`/g, '\x60')}"`//value.indexOf('$') > -1 ? `\`${value}\`` : `${JSON.stringify(value)}`;
                break;
            case 'null':
                result += null;
                break;
            ...
            case 'sum':
                ...
                if (statement.operator == '+') {
                    result += `BS.sum(${parse(statement.left)}, ${parse(statement.right)})`;
                    break;
                }
                if (statement.operator == '-') {
                    result += `BS.sub(${parse(statement.left)}, ${parse(statement.right)})`;
                    break;
                }
            ...
            case 'pow':
                ...
                if (statement.operator == '**') {
                    result += `BS.pow(${parse(statement.left)}, ${parse(statement.right)})`;
                    break;
                }
                if (statement.operator == '*') {
                    result += `BS.mul(${parse(statement.left)}, ${parse(statement.right)})`;
                    break;
                }
                if (statement.operator == '/') {
                    result += `BS.div(${parse(statement.left)}, ${parse(statement.right)})`;
                    break;
                }
                if (statement.operator == '%') {
                    result += `BS.mod(${parse(statement.left)}, ${parse(statement.right)})`;
                    break;
                }
            ...
            case 'operator_declaration':
                var t = parse(statement.arguments) || ['', ''];
                result += `
                    BS.customOperators["${parse(statement.identifier)}"] = function (${t[0]}, required = false) {
                        ${t[1]}
                        if (required && typeof arguments[0] === void 0) {
                            throw new TypeError("Missing argument at ${statement.line}:${statement.col}");
                        }
