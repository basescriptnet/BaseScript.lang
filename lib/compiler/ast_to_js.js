function include(includes) {
    ...
    for (let i = 0; i < includes.length; i++) {
        ...
        if (includes[i].type != 'include' && !(includes[i].value in libs)) {
        ...
        } else if (includes[i].type == 'include') {
            if (includes[i].addon && includes[i].addon.length) {
                result += include(includes[i].addon);
            }
            let p = pathJS(pathJS().relative());
            let filename = p.add(includes[i].value);
            let code = fs.readFileSync(filename, 'utf8');
            code = parse(to_ast(code));
            code = minify(code.includes) + '\n' + minify(code.result);
            let dirs = `const __dirname = "${p.dir.replace(/\//g, '\\\\')}", __filename = "${filename.replace(/\//g, '\\\\')}";\n`;
            result += `(function () {${dirs}${code}}).call(this);\n\n`;
            continue;
        }
        let file = libs[includes[i].value];
        let filename = pathJS(__dirname).add(`../../bin/${file}`).replace(/\//g, '\\\\');
...
const libs = {
    ...
    get builtins() {
        return '../lib/compiler/built_in.min.js';
    }
};
...
function parse (statements, tmp, isReturn, caller, prepend = '', scope) {
    ...
    for (let i = 0; i < statements.length; i++) {
        ...
        switch (statement.type) {
            case 'decorator': {
                let p = pathJS(pathJS().relative());
                let filename = pathJS(p.path).add(p.name) + baseUrl.ext;
                let dirs = `const __dirname = "${p.dir.replace(/\//g, '\\\\')}", __filename = "${filename.replace(/\//g, '\\\\')}";\n`;
                let includedCode = '';
            ...
            case '@import':
                ...
                break;
            case '@include': {
                var content = fs.readFileSync(relPath.replace(/\\/g, '/') + '/' + parse(value).slice(1, -1), 'utf8');
