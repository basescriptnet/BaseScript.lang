...
class Scopes {
    ...
    remove() {
        let last = this.last;
        this.last = this.last && this.last.parent;
        // parent remove
        if (this.last) {
...
function parse (statements, tmp, isReturn, caller, prepend = '') {
    ...
    for (let i = 0; i < statements.length; i++) {
        ...
        if (parsersObject.supported.indexOf(statement.type) !== -1) {
            let match = parsersObject.findMatch(statement, tmp, isReturn, caller, prepend, scopes);
            if (match !== void 0) {
                result += match;
                continue;
            }
        ...
        switch (statement.type) {
            ...
            case 'scope': {
                ...
                //if (Object.keys(scope.variables).length) {
                //    result += `let ${Object.keys(scope.variables).join(', ')};\n`;
                //}
                result += r;
