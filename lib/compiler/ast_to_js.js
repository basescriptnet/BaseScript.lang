    ...
    for (let i = 0; i < statements.length; i++) {
        ...
        switch (statement.type) {
            case 'decorator':
                ...
                break;
            case 'scope':
                result += `const scope${++scopeLevel} = scopes.new(${scopeLevel}, scope${scopeLevel - 1});`
                result += prependStr || '';
                if (caller == 'function')
                    result += `scope${scopeLevel}.set("args", Array.from(arguments));`
                result += `${parse(value)}
                scope${scopeLevel - 1}.scopes.pop();
                scopes.scopes.pop();`;
                scopeLevel--;
                break;
            case 'private':
                result += `(function private () {
                    ...
                    const scope${++scopeLevel} = scopes.global();
                    ${parse(value)}
                })()`;
                scopeLevel--;
                break;
            ...
            case 'var_assign':
                ...
                if (tmp == 'isGloobal')
                    pr = 'var';
                result += `${parse(value, prepend && prepend.trim ? prepend.trim() : prepend)}`;
            ...
            case 'var_reassign':
                if (tmp == 'const') {
                    result += `scope${scopeLevel}.set("${statement.identifier.value}", BS.deepFreeze(${parse(value)}), true, true)`;
                    break;
                }
                //if (tmp == 'Int') {
                ...
                if (typeof statement.identifier.value != 'string')
                    ...
                    result += `scope${scopeLevel}.set("${statement.identifier.value}", ${parse(value)}, true)`;
                break;
            case 'array':
                result += 'BS.Array([';
                for (let i = 0; i < value.length; i++) {
            ...
            case 'dot_retraction_v2':
                ...
                if (value.type !== 'identifier') {
                    result += `get_item(${from}, "${parse(value)}")`;
                    break;
                }
                result += `get_item(${from}, "${value.value}")`;
                break;
            // not used now ↓
            ...
            case 'annonymous_function': {
                ...
                res += `(${t.length ? t[0] : ''}) {
                    ${parse(statement.value, statement.text, null, 'function',
                        ...
                        +`scope${scopeLevel+1}.set("this", scope${scopeLevel+1}.variables, true, true, true);`
                        + `${t.length ? t[1] + '\n' : ''}`
                        +`${t[2] ? t[2] : ''}`)
                    }
                    return null;
                ...
                if (caller == 'statement' && statement.identifier) {
                    res = `scope${scopeLevel}.set("${statement.identifier.value}", ${res})`
                }
                if (statement.identifier) {
                    result += res;
                    break;
            ...
            case 'identifier':
                ...
                result += `scope${scopeLevel}.get("${value}")`;
                //result += value;
                break;
            case 'global':
            ...
            case 'for_loop':
                ...
                if (!/^\d+$/.test(value0) || !/^\d+$/.test(value1)) {
                    result += `for (let _i${iterator} of range(${value0}, ${value1}, ${statement.include})) {
                        ${parse(value, tmp, null, '', `scope${scopeLevel+1}.set("${statement.identifier.value}", _i${iterator}, true);`)}
                    }`
                    break;
                }
            ...
            case 'expression_short_equation': {
                ...
                if (value[0].type == 'identifier') {
                    result += `scope${scopeLevel}.set("${value[0].value}", ${v} ${value[1].value} ${parse(value[2])})`;
                    break;
                }
            ...
            case 'item_retraction':
                result += `get_item(${parse(statement.from)}, ${parse(value)})`;
                if (statement.arguments) {
                    var r = [];
                    for (let i = 0; i < statement.arguments.value.length; i++) {
            ...
            case 'arguments_with_types':
                ...
                for (let i = 0; i < sav.length; i++) {
                    ...
                    args.push(`scope${scopeLevel+1}.set("${sav[i].value}", ${sav[i].value});`)
                }
